...
ファイル、年度、前期/後期、教師名、科目名、科目群、テスト種別は必須です。年度入力欄には最初から`20`が入り、`20`から始まる半角数字4桁だけ登録できます。教師名の空白は自動で除去します。小テスト番号は任意ですが、入力する場合は半角数字だけ使えます。ファイルは保存先の`files/`へコピーされ、`localFile`に自動登録されます。取得元は自動で`手動追加`になります。コピー時のファイル名は`科目名(教師名)西暦年度.拡張子`へ変換し、小テスト番号がある場合は`_小テスト1`のように付けます。手動追加ファイルのファイル名には取得元名を付けません。同名ファイルがすでにある場合は登録せずにエラーにします。
...
手動アップロードで追加したローカルファイルは従来通り、取得元を付けない命名規則を使います。
